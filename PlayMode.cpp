...
PlayMode::PlayMode() : scene(*playarea_scene) {
	...
	for (auto &transform : scene.transforms) {
		...
		if (transform.name == "Cage") cage = &transform;
	...
	leg_tip_loop = Sound::loop_3D(*bgm_sample, 1.0f, get_leg_tip_position(), 10.0f);
}
...
glm::vec3 PlayMode::get_leg_tip_position() {
	...
	return base->make_local_to_world() * glm::vec4(-1.26137f, -11.861f, 0.0f, 1.0f);
}
